...
W0S = Scale(I2Cframe,orient=HORIZONTAL,tickinterval=32,resolution=1,from_=1,to=256,length=300,variable=var0)
...
W1S = Scale(I2Cframe,orient=HORIZONTAL,tickinterval=32,resolution=1,from_=1,to=256,length=300,variable=var1)
...
ADCframe.grid(row=5,column=1,columnspan=5,rowspan=5)
...
ppolarity = Radiobutton(ADCframe,command=pselect,text='Positive',anchor='w')
ppolarity.grid(row=2,column=5)
npolarity = Radiobutton(ADCframe,command=pselect,text='Negative',anchor='w')
npolarity.grid(row=3,column=5)

divide = Label(ADCframe,text='                ').grid(row=1,column=4,rowspan=6)

diagnostic = LabelFrame(root,text='Diagnostic Frame:')
diagnostic.grid(row=1,column=7)
errorbox = Message(diagnostic,text='Error 404 nothing to display here except an error that is long!',bg='white',relief=RIDGE,width=300)
